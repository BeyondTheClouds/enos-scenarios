...
flyon:~$ kadeploy3 -f $OAR_NODE_FILE -e debian9-x64-std -k
...
#+BEGIN_EXAMPLE
root@taurus-13:~# git clone https://github.com/BeyondTheClouds/enos.git -b stable/pike
root@taurus-13:~# cd enos
root@taurus-13:~/enos#: virtualenv --python=python2 venv
root@taurus-13:~/enos#: source venv/bin/activate
(venv) root@taurus-13:~/enos# pip install -e .
#+END_EXAMPLE
...
#+BEGIN_EXAMPLE
(venv) root@taurus-13:~/enos# enos --help
...
: (venv) root@taurus-13:~/enos# enos new > reservation.yaml
...
: vagrant@enos-node:~/enos/$ enos deploy -f reservation.yaml

Then, you can observe EnOS deploying containers on one of the deployed node from
another terminal of your VM with:
: vagrant@compute-node:$ sudo docker ps

** Play with OpenStack
The last service deployed is the OpenStack dashboard (Horizon). Once
the deployment process is finished, Horizon is reachable from G5K. To access
Horizon from your web browser, you need to create an SSH tunnel from your
laptop, and G5K. To that end, open a new terminal and type the following:
: user@laptop~$ ssh -NL 8000:<g5k_node>:80 <g5k_site>.g5k
: user@laptop~$ ssh -NL 8000:sagittaire-44-kavlan-4.lyon.grid5000.fr:80 nancy.g5k

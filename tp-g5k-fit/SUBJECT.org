...
Get a new machine on G5K:
#+BEGIN_EXAMPLE
flyon:~$ oarsub -I -l "nodes=1,walltime=4:00:00" -p "cluster='taurus'"
#+END_EXAMPLE

Download EnOS in your working directory, and install it:
#+BEGIN_EXAMPLE
user@taurus-13:~# git clone https://github.com/BeyondTheClouds/enos.git -b stable/pike
user@taurus-13:~# cd enos
user@taurus-13:~/enos#: virtualenv --python=python2 venv
user@taurus-13:~/enos#: source venv/bin/activate
(venv) user@taurus-13:~/enos# pip install -e .
#+END_EXAMPLE

#+BEGIN_NOTE
The G5K environment contains EnOS together with all resources needed by EnOS to
run OpenStack (included all the Docker container images). The result is
relatively huge, but it contains everything you need to run EnOS almost offline.
#+END_NOTE

#+BEGIN_EXAMPLE
(venv) user@taurus-13:~/enos# enos --help
...
: (venv) user@taurus-13:~/enos# enos new > reservation.yaml
...
In this example, we declare 3 machines on the ~paravance~ cluster (which is
...
deployment process is finished, Horizon is reachable from G5K. To access Horizon
from your own web browser (from your laptop), you need to create an SSH tunnel
from your laptop to the control node, located in G5K. To that end, your need
first to find out the the control node address, and then create the tunnel. Open n a
new terminal and type the following:
1. Find the control node address using EnOS:
: (venv) user@frennes~/enos$ enos info
: (venv) user@frennes~/enos$ enos info --out json | jq -r '.rsc.control[0].address'
2. Create the tunnel from your laptop:
: user@laptop~$ ssh -NL 8000:<g5k_control_node>:80 <g5k_site>.g5k
: user@laptop~$ ssh -NL 8000:sagittaire-44-kavlan-4.lyon.grid5000.fr:80 nancy.g5k -v
...
% This is not working through the tunnel:
...
#+BEGIN_NOTE
This exercice has been designed to *run on a cluster where nodes have
two network interfaces*. *If you plan to run the execice on a cluster
with single network interface, please run the following script on the
network node.* You can check how many network interfaces are
associated to a cluster by consulting the
[[https://www.grid5000.fr/mediawiki/images/G5k_cheat_sheet.pdf][Grid'5000
Cheatsheet]]
#+END_NOTE
...
: user@laptop~$ ssh -NL 3000:sagittaire-44-kavlan-4.lyon.grid5000.fr:3000 nancy.g5k
...
#+BEGIN_NOTE
Note that Shaker workload is [[(disabled)][disabled for now.
#+END_NOTE

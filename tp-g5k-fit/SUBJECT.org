...
network, storage resources). To that end, it provide management mechanisms as a
...
Reserve and provision a new machine on G5K, and SSH to it:
#+BEGIN_EXAMPLE
flyon:~$ oarsub -I -l "nodes=1,walltime=4:00:00" -p "cluster='taurus'" -t deploy
flyon:~$ kadeploy3 -f $OAR_NODE_FILE -e debian9-x64-enos -u discovery -k
flyon:~$ enos_node=$(uniq ${OAR_NODE_FILE})
flyon:~$ ssh ${enos_node}
#+END_EXAMPLE

Download EnOS in your working directory, and install it:
#+BEGIN_EXAMPLE
user@taurus-13:~# git clone https://github.com/dpertin/enos.git -b stable/pike
...
: user@enos-node:~/enos$ ping <floating-ip> # floating-ip is 192.168.143.*
...
: user@enos-node:~/enos$ enos backup --backup_dir=/vagrant_data
...
#+BEGIN_EXAMPLE
user@enos-node:~/enos$ enos info --out json

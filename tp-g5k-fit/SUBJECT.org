...
1. a machine to run EnOS, we call ~enos-node~ in the rest of the document;
...
: user@laptop:~$ curl  http://enos.irisa.fr/tp-g5k/grafana_dashboard.json -O /tmp/
...
=/tmp/grafana_dashboard.json= file. Next, make
...
#+BEGIN_NOTE
For those interested in playing with deploying applications on top of OpenStack,
you can jump to another workshop involving Heat: the OpenStack Orchestration
service [[http://enos.irisa.fr/tp-g5k/HEAT-SUBJECT.html][here]].
#+END_NOTE
...
Since our topology is now defined by groups, we need to run ~enos deploy~ first
(which should be faster than the first time), then, enforce these constraints
with ~enos tc~, which results in:
...
reconfigure Neutron:
: user@enos-node:~/enos$ enos os --tags=neutron --reconfigure

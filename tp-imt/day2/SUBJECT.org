...
- [[#requirements-and-setup][Requirements and Setup]]
  ...
  - [[#finish-the-initialization-of-openstack][Finish the initialization of OpenStack]]
    ...
    - [[#ping-vm-from-the-frontend][Ping VM from the Frontend]]
...
#+INCLUDE: "lib/os-control-compute.sh" src bash
...
#+INCLUDE: "lib/os-compute.sh" src bash
...
#+BEGIN_SRC bash
debian@os_control_compute$ sudo iptables -t nat -A PREROUTING\
...
({{{ie}}}, ~debian@os_control_compute~), then add a route that goes
through the router.
...
the Play with OpenStack section of the previous session (see
[[file:../day1/SUBJECT.org::#sec:play-with-os][Day 1 → Sec 3]]).
Horizon is available behind the public IP of ~os_control_compute~.
#+END_NOTE
...
#+BEGIN_SRC bash
debian@os_control_compute:~$ for vm in $(openstack server list -c Name -f value); do\
  echo "Delete ${vm}...";\
...
#+BEGIN_SRC bash
debian@os_control_compute:~$ for ip in $(openstack floating ip list -c "Floating IP Address" -f value); do\
  echo "Delete ${ip}...";\
...
#+BEGIN_SRC bash
debian@os_control_compute:~$ openstack server add floating ip\
  wordpress-wp\
...
#+BEGIN_SRC bash
debian@os_control_compute:~$ lynx <floating_ip>/wp
#+END_SRC

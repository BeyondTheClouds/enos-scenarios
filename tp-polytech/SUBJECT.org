...
*** Get the base box
Download the Vagrant box[fn:enos-box] in your working directory.
: host:~$ mkdir omh-co
: host:~$ cd omh-co
: host:~/omh-co$ curl -O http://enos.irisa.fr/vagrant-box/tp.box

#+BEGIN_NOTE
The box contains EnOS together with all resources needed by EnOS to
run OpenStack. The result is a relatively huge box (~3GB), but it

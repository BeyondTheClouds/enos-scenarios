...
*** Get the base box
Download the Polytech Vagrant box[fn:enos-box] in your working
directory.
: host:~$ mkdir oph-co
: host:~$ cd oph-co
: host:~/oph-co$ curl -O http://enos.irisa.fr/vagrant-box/polytech.box

#+BEGIN_NOTE
The box contains EnOS together with all resources needed by EnOS to
run OpenStack. The result is a relatively huge box (~6GB), but it
...
Vagrant.configure("2") do |config|
  ...
  config.vm.box = "file://./polytech.box"
...
machine if you want it to be pingable from the lab.
#+BEGIN_EXAMPLE
...
Provisionning automatically installs software, alters configurations,
